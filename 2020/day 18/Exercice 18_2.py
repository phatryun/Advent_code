...
def getFreeOp(str_exp) :
    list_add = [i for i in range(len(str_exp)) if str_exp[i] == '+']
    list_mult = [i for i in range(len(str_exp)) if str_exp[i] == '*']
    ...
    for op in list_mult :
        ...
        if free_op :
            list_free_mult.append(op)

    return list_free_add, list_free_mult
...
class Expression :
    ...
    def __init__(self, str_exp):
        ...
        if str_exp.isnumeric() :
            ...
            free_add, free_mult = getFreeOp(str_exp)
            if len(free_mult) > 0 :
                self.op = str_exp[max(free_mult)]
                self.left = Expression(str_exp[:max(free_mult) - 1])
                self.right = Expression(str_exp[max(free_mult) + 2:])
            elif len(free_add) > 0 :
                self.op = str_exp[max(free_add)]
                self.left = Expression(str_exp[:max(free_add) - 1])
                self.right = Expression(str_exp[max(free_add) + 2:])
